...
| `GET /api/v1/strategies/{id}/safety` | Composite safety status (viewer+; read class): lifecycle state, `paused_from`, binding kills with clearing events, `active_kill`, breaker-today, watchdog liveness. Normative in `docs/specs/operator-surface.md` (OS-5..OS-14). |
| `GET /api/v1/strategies/{id}/alerts?page&limit` | Per-strategy `safety_alerts` feed (viewer+; read class), newest first (`recorded_at DESC, alert_id DESC`). Normative in operator-surface.md (OS-15..OS-18). |
| `GET /api/v1/strategies/{id}/limits` | Effective runtime risk limits (viewer+; read class; registered iff a limits provider is wired): `{effective, changeable_fields, changes}` — the provider's overlay view (decimal fields as ADR-0003 strings), the sorted runtime-changeable whitelist, and this strategy's `risk_limit_changes` audit rows in rowid order. Normative in `docs/specs/multi-tenant-rbac.md` §Runtime limit changes. |
